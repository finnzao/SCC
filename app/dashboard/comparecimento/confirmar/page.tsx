    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-3xl mx-auto p-4 md:p-6">
        <div className="flex items-center gap-3 mb-6">
          <button onClick={() => h.router.back()} className="text-gray-500 hover:text-gray-800 transition-colors">
          ...
          <StepIndicator currentStep={h.currentStep} totalSteps={4} />
        </div>
          ...
          <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-6 mb-6">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-emerald-700 text-sm">{h.mensagem}</p>
                {h.proximoComparecimento && (
                  <div className="mt-3 bg-white rounded-lg p-3 border border-emerald-200">
                    ...
                    <p className="text-base font-semibold text-primary">{h.proximoComparecimento}</p>
                  </div>
                )}
          ...
          <div className="bg-red-50 border border-red-200 rounded-xl p-6 mb-6">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-red-700 text-sm mb-3">{h.mensagem}</p>
                <button onClick={() => h.setEstado('inicial')} className="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 text-sm">Tentar Novamente</button>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="space-y-5">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="px-5 py-3.5 border-b border-gray-100">
                  <h2 className="text-sm font-semibold text-gray-800 flex items-center gap-2">
                    <Search className="w-4 h-4 text-primary" />Buscar Pessoa
                  </h2>
                </div>
                <div className="p-5 space-y-3">
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      ...
                      <input type="text" value={h.buscaProcesso} onChange={(e) => h.setBuscaProcesso(e.target.value)}
                        onKeyDown={(e) => e.key === 'Enter' && h.buscarPessoa()}
                    ...
                    <button onClick={h.buscarPessoa} disabled={h.estado === 'buscando'}
                      ...
                      {h.estado === 'buscando' ? 'Buscando...' : 'Buscar'}
                    </button>
                  </div>
            ...
            {h.custodiado && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="px-5 py-3.5 border-b border-gray-100">
                  <h2 className="text-sm font-semibold text-gray-800 flex items-center gap-2">
                    <User className="w-4 h-4 text-primary" />Pessoa Selecionada
                  </h2>
                </div>
                <div className="p-5">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-gray-800">{h.custodiado.nome}</p>
                      <p className="text-xs text-gray-500">{h.custodiado.cpf || 'CPF não informado'} · {h.custodiado.rg || 'RG não informado'}</p>
                    </div>
                  </div>
                  {h.processoSelecionado && (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 flex items-center justify-between">
                      <div>
                        <p className="text-xs text-blue-600 font-medium">Processo selecionado</p>
                        <p className="font-mono text-sm text-blue-900">{h.processoSelecionado.numeroProcesso}</p>
                        <p className="text-xs text-blue-700">{h.processoSelecionado.vara} · {h.processoSelecionado.comarca} · {h.formatarPeriodicidade(h.processoSelecionado.periodicidade)}</p>
                      </div>
                      {h.temMultiplosProcessos && (
                        <button onClick={h.trocarProcesso} className="text-xs text-blue-600 hover:text-blue-800 flex items-center gap-1 flex-shrink-0 font-medium">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-blue-200 overflow-hidden">
                <div className="px-5 py-3.5 border-b border-blue-100 bg-blue-50/50">
                  <h2 className="text-sm font-semibold text-blue-800 flex items-center gap-2">
                    <FileText className="w-4 h-4" />Selecione o Processo
                  </h2>
                  <p className="text-xs text-blue-600 mt-0.5">
                    Este custodiado possui {h.processosDisponiveis.length} processos ativos. Escolha para qual deseja registrar.
                  </p>
                </div>
                <div className="p-5">
                  <SeletorProcesso processos={h.processosDisponiveis} selecionado={h.processoSelecionado} onSelecionar={h.selecionarProcesso} />
                </div>
              </div>
            )}

            {h.mostrarAvisoSemProcesso && (
            ...
            {h.mostrarFormularioCompleto && (
              <>
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                  <div className="px-5 py-3.5 border-b border-gray-100">
                    <h2 className="text-sm font-semibold text-gray-800 flex items-center gap-2">
                      <MapPin className="w-4 h-4 text-primary" />Verificação de Endereço
                    </h2>
                  </div>
                  <div className="p-5">
                    {!h.enderecoRespondido ? (
                      <div className="space-y-3">
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                          <p className="text-sm text-blue-800 font-medium mb-2">O endereço cadastrado está correto?</p>
                          {h.custodiado?.endereco ? (
                            <div className="text-sm text-blue-700 space-y-0.5">
                              <p>{h.custodiado.endereco.logradouro}{h.custodiado.endereco.numero ? `, ${h.custodiado.endereco.numero}` : ''}</p>
                              <p>{h.custodiado.endereco.bairro} · {h.custodiado.endereco.cidade} - {h.custodiado.endereco.estado}</p>
                              <p>CEP: {h.custodiado.endereco.cep}</p>
                            </div>
                          ) : (<p className="text-sm text-blue-600 italic">Nenhum endereço cadastrado</p>)}
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                          <button onClick={h.confirmarEnderecoCorreto}
                          ...
                          <button onClick={h.indicarMudancaEndereco}
                    ...
                    ) : h.atualizacaoEndereco.houveAlteracao ? (
                      <div>
                        ...
                        <div className="mt-4">
                          ...
                          <textarea value={h.atualizacaoEndereco.motivoAlteracao || ''}
                            onChange={(e) => h.setMotivoAlteracao(e.target.value)}
                        ...
                        <button onClick={h.resetarRespostaEndereco}
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                  <div className="px-5 py-3.5 border-b border-gray-100">
                    <h2 className="text-sm font-semibold text-gray-800 flex items-center gap-2">
                      <FileText className="w-4 h-4 text-primary" />Dados do Comparecimento
                    </h2>
                  </div>
                  <div className="p-5 space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <select value={h.formulario.tipoValidacao}
                          onChange={(e) => h.handleInputChange('tipoValidacao', e.target.value)}
                      ...
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input type="text" value={h.formulario.validadoPor} readOnly
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input type="date" value={h.formulario.dataComparecimento}
                      ...
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input type="time" value={h.formulario.horaComparecimento}
                            onChange={(e) => h.handleInputChange('horaComparecimento', e.target.value)}
                            className="w-full pl-9 pr-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-transparent" />
                        </div>
                      </div>
                    </div>
                    <div>
                      ...
                      <textarea value={h.formulario.observacoes}
                        onChange={(e) => h.handleInputChange('observacoes', e.target.value)}
                        rows={2} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm resize-none focus:ring-2 focus:ring-primary focus:border-transparent"
                ...
                <div className="flex items-center gap-3 pt-2 pb-20 md:pb-2">
                  ...
                  <button onClick={h.confirmarComparecimento} disabled={!h.podeConfirmar}
            ...
            {h.estado === 'inicial' && !h.custodiado && !processoIdParam && !custodiadoIdParam && (
